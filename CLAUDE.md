...
- `scripts/sync_exclude.txt` — ASINs (one per line) that `sync_store.py` must
  never touch
- `products.json` — generated by `sync_store.py`, committed; the server-side
  product list `create-payment-intent.js` validates cart items against — see
...
- Looks up each cart item's price server-side by `slug` (see "Server-side price
  validation" below) and sums `price * qty` itself, rather than trusting a
  client-sent total, then adds shipping (`SHIPPING_FLAT_CENTS` unless the item
  total meets `FREE_SHIPPING_THRESHOLD_CENTS`) computed the same way — the
  client never sends a shipping, price, or total value.
...
## Server-side price validation
`scripts/sync_store.py` writes `products.json` at the repo root (a flat array
of `{slug, asin, name, price}`, sorted by slug) as the last step of every
real (non-`--dry-run`) run, via `write_products_json()` — it re-scans every
`product-<slug>.html` after all creates/price-updates/deletes for that run
have been applied, so it always reflects final on-disk state, not a stale
snapshot. Commit this file; it's product data, not a secret.

`netlify.toml` bundles it with every function (`[functions].included_files =
["products.json"]`), and `create-payment-intent.js` loads it
(`path.join(__dirname, '..', '..', 'products.json')`, cached per warm Lambda
instance) to validate the cart the client sent:
- `checkout.html` sends only `{slug, qty}` per item — no `name` or `price` at
  all, since neither is trusted from the client anymore.
- An item whose `slug` isn't in `products.json` is rejected outright
  (`"Unknown product: <slug>"`) rather than silently priced at whatever the
  client sent.
- The price used for both the Stripe Tax line item and the PaymentIntent
  amount always comes from `products.json`, never from the request body.

This closes the gap the original "TODO before going live" comment flagged: a
caller editing `localStorage.fktrade_cart` (or hitting the endpoint directly)
can no longer submit an arbitrary price or invent a nonexistent product —
the worst they can do is order a real product, at its real price, in a
quantity they choose.
